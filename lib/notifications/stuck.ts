...
export async function runStuckCheck(now: Date = new Date()): Promise<void> {
  ...
  for (const event of events) {
    ...
    if (ageDays >= 14) {
      ...
      for (const admin of admins) {
        await notifyOnce(
          admin.id,
          NOTIFICATION_KIND.EVENT_STUCK_ADMIN,
          `stuck-admin:${event.id}:${event.stageChangedAt.getTime()}`,
          text,
          now
        );
      }
      ...
      await notifyOnce(
        event.leadId,
        NOTIFICATION_KIND.EVENT_STUCK_LEAD,
        `stuck-lead:${event.id}:${event.stageChangedAt.getTime()}`,
        text,
        now
      );
    }
